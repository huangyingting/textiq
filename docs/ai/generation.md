...
| Mobile visual insertion | [`src/app/app/documents/[id]/mobile-generate-visual-section.tsx`](../../src/app/app/documents/%5Bid%5D/mobile-generate-visual-section.tsx) |
...
source block key and source text captured when its request began, so a later
selection move cannot re-anchor the visual under unrelated text or change its
provenance. A failed regeneration leaves the prior candidates bound to their
prior source; only a successful replacement moves that binding. If that source
block is deleted before insertion, the candidate is not silently discarded or
inserted elsewhere; the surface keeps recoverable feedback and offers
generation against the current selection.
...
- [`src/app/app/documents/[id]/mobile-generate-visual-section.test.tsx`](../../src/app/app/documents/%5Bid%5D/mobile-generate-visual-section.test.tsx)
