...
the image-retry tier for gradient, pattern, and image fills that cannot stay as a
solid fill.
...
| Shapes                 | Core shapes render; path fallback styling is deterministic.  | Core shapes map to PptxGenJS shapes; solid fills stay native; gradient, pattern, and image fills use image-retry fills with stroke overlays where needed.                                                                         | Unsupported native fill mappings and effects diagnose.                                   |
