...
| Medium   | Broad Playwright coverage outside the deterministic profile is opt-in. | `.github/workflows/e2e-deterministic.yml` hard-gates the seeded deterministic profile; `e2e/README.md` keeps the broader Playwright suite local/opt-in.                                                                                                        | Browser regressions outside the seeded profile can still require manual or subsystem-specific runs. | Promote additional specs deliberately when they become release-blocking; keep the UI matrix inventory tracked separately.                                   |
...
1. [ASK USER] Which optional Playwright specs outside the deterministic profile should become future release-blocking checks?
2. [ASK USER] Should future production slide-asset storage use a cloud adapter such as S3/Azure Blob, or is local filesystem storage acceptable for current deployments?
