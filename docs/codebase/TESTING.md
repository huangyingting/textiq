...
| Deterministic E2E profile | Yes, required in CI  | Seeded profile specs for document editor, import, present/export, asset access, layout rendering.                   | `.github/workflows/e2e-deterministic.yml` runs on PR/push through the no-build dev-server profile runner with `E2E_PROFILE_GREP=@required-profile` and fails the workflow on profile failures. |
