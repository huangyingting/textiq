...
Last content update: 2026-07-20 — conditional abuse-budget rate limits are documented for account export and brand assets.
...
| `brand-assets/[ownerId]/[...path]`    | `authenticated-session` | Required              | Per client IP                   | Session `user.id` must equal the `ownerId` partition (`decideBrandAssetAccess`)                                              | 429 plain `Too many requests` (+`Retry-After`, only when `AUTH_SECRET` is set); 404 plain `Not found` for missing, anonymous, or non-owner requests; 200 bytes                                                                      | Binary/plain-text           | Brand            | Serves private brand logo/font bytes (Epic #496). Owner-scoped; every denial is a privacy 404 so callers cannot enumerate private asset keys. Abuse budget `public.asset.ip` applies when `AUTH_SECRET` is set. |
