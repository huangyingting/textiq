...
Last content update: 2026-07-20 — brand-asset denials are uniformly privacy 404.
...
| `brand-assets/[ownerId]/[...path]`    | `authenticated-session` | Required              | No                              | Session `user.id` must equal the `ownerId` partition (`decideBrandAssetAccess`)                                              | 404 plain `Not found` for missing, anonymous, or non-owner requests; 200 bytes                                                                                                                                                      | Binary/plain-text           | Brand            | Serves private brand logo/font bytes (Epic #496). Owner-scoped; every denial is a privacy 404 so callers cannot enumerate private asset keys.                                                                   |
