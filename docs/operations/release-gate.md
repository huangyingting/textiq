...
- The profile credential harness is fail-stop. Chromium pins the per-run HTTPS/
  WSS proxy SPKI, Node helpers trust the same certificate, and the anonymous
  private-key descriptor is inherited only by the dedicated proxy process, not
  Playwright or its descendants, and never enters files, env values, argv, or
  logs. Request
  capabilities bind method/origin/Host/path/query/body hash/TLS channel/time/
  nonce and are atomically one-use. The proxy proves the accepted app socket's
  endpoint and process ownership before writing upstream; hostile takeover tests
  require zero credential bytes. Upstream redirects are returned only after
  singular internal-app `Location` values are rewritten to the pinned public
  HTTPS origin.

| Spec (Epic #517)                                     | Covers                                                                                                                                                                     |
| ---------------------------------------------------- | -------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `e2e/documents/template-creation.spec.ts`            | Dashboard failure/retry, duplicate suppression, modal focus/scroll/mobile behavior, durable template creation, and workspace role enforcement                              |
| `e2e/import/import-roundtrip.spec.ts`                | #519 Markdown failure/retry + deterministic DOCX dashboard import; workspace UI import; editor replacement modal/autosave/reload; role enforcement; unsupported-type error |
...
| D-5 | Document import (markdown, .docx) | **A** + **E2E** | Required deterministic CI covers Markdown failure/retry, duplicate suppression, editor replacement modal/autosave/reload, workspace UI import, and generated-fixture DOCX round-trips in `e2e/import/import-roundtrip.spec.ts` (#519/#1734)  |
| D-6 | Document search                   | **A**           | `search.test.ts`                                                                                                                                                                                                                             |
| D-7 | Document delete / trash / restore | **A** + **E2E** | Service and UI tests cover soft-delete, restore, permanent deletion, failure retry, duplicate suppression, pending dismissal lock, and final-row empty state; deterministic browser coverage proves the successful persisted lifecycle       |
...
| D-9 | Document create from template     | **A** + **E2E** | Required deterministic CI covers dashboard failure/retry, duplicate suppression, modal accessibility/mobile containment, durable reload persistence, and workspace owner/editor/viewer behavior in `e2e/documents/template-creation.spec.ts` |
...
| S-5 | Present mode (read-only render)     | **M** + **E2E** | Authenticated + public present are asserted in required CI by `e2e/presentation/present-export.spec.ts`; both rendered surfaces are retained in the `release-review-<commit SHA>` artifact for manual inspection (#520)                                                                                                                                                                                                                                                                             |
...
| S-8 | Slide editor responsive layout      | **A** + **E2E** | Deterministic presentation layout rendering in `e2e/presentation/slides-layout-screenshots.spec.ts` (#1449); pixel snapshot comparisons opt-in via `E2E_SLIDES_LAYOUT_SCREENSHOTS=1`. Required-profile assertions prove fitted title text, unobstructed selection and Text-inspector routing across desktop/tablet/mobile, while `e2e/presentation/focus-and-mobile-controls-regression.spec.ts` proves the non-desktop inspector sheet owns hit-testing above the intersecting filmstrip and dock. |
...
| #    | Flow                             | Owner           | Notes                                                                                                                                                                                                                                                                       |
| ---- | -------------------------------- | --------------- | --------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| AS-1 | Image upload and protected asset | **M** + **E2E** | Required CI exercises inspector upload, autosave, reload, public rendering, and protected-asset access in `e2e/presentation/slide-asset-upload.spec.ts`; the rendered uploaded slide is retained in the `release-review-<commit SHA>` artifact for manual inspection (#521) |
...
3. Download the deterministic E2E workflow's `release-review-<commit SHA>`
   ...
   responsive-layering PNGs, then walk the checklist in Part 2 and confirm every
   **M** flow manually.
