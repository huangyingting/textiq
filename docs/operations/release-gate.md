...
| S-8 | Slide editor responsive layout      | **A** + **E2E** | Deterministic presentation layout rendering in `e2e/presentation/slides-layout-screenshots.spec.ts` (#1449); pixel snapshot comparisons opt-in via `E2E_SLIDES_LAYOUT_SCREENSHOTS=1`. The required-profile mobile/tablet inspector regressions in `e2e/presentation/focus-and-mobile-controls-regression.spec.ts` prove the sheet owns hit-testing above the intersecting filmstrip and dock across the non-desktop breakpoint. |
