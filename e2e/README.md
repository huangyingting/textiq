...
| `e2e/presentation/pointer-interactions.spec.ts`         | Real pointer drag coverage for filmstrip reorder, node transforms, connector snapping, image crop/history, and persistence        |
...
commands, history, autosave, reload persistence, workspace landmarks and
creation controls, and accessible slide-toolbar controls. Under
`E2E_PROFILE=1`, every authenticated case uses the seeded owner and document;
none of those cases skip for missing ad hoc credentials. Outside the profile,
the spec degrades cleanly:

- Without `E2E_USER_EMAIL`/`E2E_USER_PASSWORD`, authenticated external-fixture
  cases skip.
- Without `E2E_SLIDES_DOC_URL`, external document smoke cases skip.
