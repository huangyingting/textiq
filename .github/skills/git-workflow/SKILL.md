---
name: "git-workflow"
description: "Main-only Squad repository workflow"
...
TextIQ uses a **main-only** repository workflow. `main` is the only branch that feature work targets for merge; there is no `dev`, `preview`, `insider`, or `insiders` promotion branch in the active workflow.

| Branch | Purpose |
| ------ | ------- |
| `main` | Integration, release, and production-ready branch. All PRs target this branch. |

Legacy remote branches such as `dev`, `preview`, `insider`, or `insiders` may still exist. Do not target them and do not delete them unless a human explicitly requests that cleanup.
...
**Important:** Remove local links before committing. `npm link` and `go replace` are local-only — CI must use published packages or PR-specific refs.
...
- ❌ Branching from `dev`, `preview`, `insider`, or `insiders` for TextIQ work (branch from `main`)
- ❌ PR targeting `dev`, `preview`, `insider`, or `insiders` (target `main`)
- ❌ Non-conforming branch names (must be `squad/{number}-{slug}`)
- ❌ Committing directly to `main` (use PRs)
- ❌ Deleting legacy remote `dev`/`insider`/`insiders` branches without explicit human approval
...
- Normal work: feature branch → PR to `main` → merge after review and required checks.
- Releases: tag or release from `main` after the TextIQ release gate passes.
- Hotfixes: branch from `main`, PR back to `main`, then tag/release if needed.
- No promotion pipeline: do not run `dev → preview`, `dev → insiders`, or `preview → main` flows for TextIQ.
