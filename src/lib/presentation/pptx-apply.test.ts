/**
 * Unit tests for the presentation PPTX applier.
 *
 * Pure helpers (`textContentToPptxRuns`, `presentationShapeToName`) are tested
...
} from "@/lib/presentation/pptx-apply";
import type { PptxTextRun } from "@/lib/presentation/pptx-apply";
import type {
  PptxTextOp,
  PptxShapeOp,
  PptxTableOp,
  ...
  PptxVisualOp,
  PptxDeckSpec,
} from "@/lib/presentation/pptx-export-adapter";
import type { TextContent, TableContent } from "@/lib/presentation/schema";
import {
  buildContentSlide,
  buildDeck,
  buildImageAsset,
...
describe("resolveExportSpecAssetSources", () => {
  test("replaces image operation asset ids with Deck image src values", () => {
...
describe("presentationShapeToName", () => {
  test("known shapes map to pptxgenjs names", () => {
    assert.equal(presentationShapeToName("rect"), "rect");
    assert.equal(presentationShapeToName("ellipse"), "ellipse");
    assert.equal(presentationShapeToName("circle"), "ellipse");
    assert.equal(presentationShapeToName("line"), "line");
    assert.equal(presentationShapeToName("triangle"), "triangle");
    assert.equal(presentationShapeToName("diamond"), "diamond");
    assert.equal(presentationShapeToName("roundRect"), "roundRect");
  });

  test("unknown shape falls back to rect", () => {
    assert.equal(presentationShapeToName("hexagon"), "rect");
    assert.equal(presentationShapeToName(""), "rect");
  });
});
...
describe("applyPptxTextOp", () => {
  function makeTextOp(overrides: Partial<PptxTextOp> = {}): PptxTextOp {
    return {
      type: "text",
      id: "t1",
      x: 1,
  ...
  test("calls addText on the mock slide", () => {
    ...
    applyPptxTextOp(slide as never, makeTextOp());
  ...
  test("passes x, y, w, h dimensions", () => {
    const { slide, calls } = makeMockSlide();
    applyPptxTextOp(slide as never, makeTextOp({ x: 2, y: 1, w: 5, h: 2 }));
  ...
  test("rotation is mapped to rotate", () => {
    const { slide, calls } = makeMockSlide();
    applyPptxTextOp(slide as never, makeTextOp({ rotation: 45 }));
    const opts = calls[0].args[1] as Record<string, unknown>;
  ...
  test("multi-paragraph content uses run array form", () => {
    ...
    applyPptxTextOp(slide as never, op);
...
describe("applyPptxShapeOp", () => {
  function makeShapeOp(overrides: Partial<PptxShapeOp> = {}): PptxShapeOp {
    return {
      type: "shape",
      id: "s1",
      shape: "rect",
  ...
  test("calls addShape on the mock slide", () => {
    const { slide, calls } = makeMockSlide();
    applyPptxShapeOp(slide as never, makeShapeOp());
    assert.ok(calls.some((c) => c.kind === "addShape"));
  });

  test("circle shape maps to ellipse", () => {
    const { slide, calls } = makeMockSlide();
    applyPptxShapeOp(slide as never, makeShapeOp({ shape: "circle" }));
  ...
  test("fill color is forwarded as fill.color", () => {
    const { slide, calls } = makeMockSlide();
    applyPptxShapeOp(slide as never, makeShapeOp({ fill: "AABBCC" }));
    const opts = calls[0].args[1] as Record<string, unknown>;
...
describe("applyPptxTableOp", () => {
  ...
  function makeTableOp(overrides: Partial<PptxTableOp> = {}): PptxTableOp {
    return {
      type: "tableShape",
      id: "tbl1",
      x: 1,
  ...
  test("calls addTable on the mock slide", () => {
    const { slide, calls } = makeMockSlide();
    applyPptxTableOp(slide as never, makeTableOp());
    assert.equal(calls.length, 1);
  ...
  test("headerFill is applied to header cells", () => {
    const { slide, calls } = makeMockSlide();
    applyPptxTableOp(slide as never, makeTableOp({ headerFill: "003399" }));
    const rows = calls[0].args[0] as Array<
  ...
  test("rowFill is applied to data cells", () => {
    const { slide, calls } = makeMockSlide();
    applyPptxTableOp(slide as never, makeTableOp({ rowFill: "EEEEEE" }));
    const rows = calls[0].args[0] as Array<
      Array<{ options?: Record<string, unknown> }>
    >;
  ...
  test("passes x, y, w, h to addTable options", () => {
    const { slide, calls } = makeMockSlide();
    applyPptxTableOp(slide as never, makeTableOp({ x: 2, y: 3, w: 9, h: 5 }));
    const opts = calls[0].args[1] as Record<string, unknown>;
    assert.equal(opts.x, 2);
...
describe("applyPptxConnectorOp", () => {
  function makeConnectorOp(
    overrides: Partial<PptxConnectorOp> = {},
  ): PptxConnectorOp {
    return {
      type: "connector",
      id: "con1",
  ...
  test("calls addShape with line", () => {
    const { slide, calls } = makeMockSlide();
    applyPptxConnectorOp(slide as never, makeConnectorOp());
    assert.equal(calls.length, 1);
...
describe("applyPptxImageOp", () => {
  function makeImageOp(overrides: Partial<PptxImageOp> = {}): PptxImageOp {
    return {
      type: "image",
      id: "img1",
  ...
  test("calls addImage with path source for non-data URI", async () => {
    const { slide, calls } = makeMockSlide();
    await applyPptxImageOp(slide as never, makeImageOp());
    assert.equal(calls.length, 1);
    assert.equal(calls[0].kind, "addImage");
  ...
  test("calls addImage with data source for data URI", async () => {
    ...
    await applyPptxImageOp(slide as never, makeImageOp({ assetId: dataUri }));
    assert.equal(calls.length, 1);
    const opts = calls[0].args[0] as Record<string, unknown>;
  ...
  test("forwards contain fit as PPTX image sizing when no crop is present", async () => {
    const { slide, calls } = makeMockSlide();
    await applyPptxImageOp(slide as never, makeImageOp({ fit: "contain" }));
    const opts = calls[0].args[0] as Record<string, unknown>;
    assert.deepEqual(opts.sizing, {
      type: "contain",
      w: 4,
...
describe("applyPptxVisualOp", () => {
  function makeVisualOp(overrides: Partial<PptxVisualOp> = {}): PptxVisualOp {
    return {
      type: "visual",
      id: "visual1",
      visualId: "doc-visual-1",
  ...
  test("renders a deterministic placeholder with channel colors", async () => {
    const { slide, calls } = makeMockSlide();
    await applyPptxVisualOp(slide as never, makeVisualOp());
...
describe("PPTX assembly boundaries", () => {
  test("applyPptxSpec assembles slides, dispatches ops, and includes notes", async () => {
    const spec: PptxDeckSpec = {
      layout: "LAYOUT_WIDE",
      slideW: 13.333,
      slideH: 7.5,
      diagnostics: [],
    ...
    const blob = await applyPptxSpec(spec);
  ...
  test("exportDeckAsPPTX runs the presentation export pipeline with an explicit theme package", async () => {
    const deck = buildDeck([buildContentSlide("Pipeline")], {
      theme: { packageId: "test-package" },
    });

    const blob = await exportDeckAsPPTX(deck, buildMinimalThemePackage(), {
      canvasWidthPx: 960,
      canvasHeightPx: 540,
    });
