...
function addImageFill(
  slide: PptxSlide,
  fill: PptxImageFill,
  op: Pick<PptxShapeOp, "x" | "y" | "w" | "h" | "rotation">,
  shadow?: PptxShadow,
): void {
  slide.addImage({
...
/**
 * Maps a presentation shape name string to a PptxGenJS shape name.
 * Falls back to `"rect"` for unknown shapes.
 */
export function presentationShapeToName(shape: string): string {
...
export function applyPptxShapeOp(slide: PptxSlide, op: PptxShapeOp): void {
  const { x, y, w, h, shape, fill, stroke, rotation } = op;
  const shadow = effectToPptxShadow(op.effect);
  const shapeName = presentationShapeToName(shape) as Parameters<
    PptxSlide["addShape"]
  >[0];
...
type ConnectorDash = NonNullable<PptxConnectorOp["stroke"]>["dash"];
...
function arrowToPptxArrow(
  arrow: PptxConnectorOp["startArrow"],
): "none" | "arrow" | "triangle" {
...
export function applyPptxConnectorOp(
  slide: PptxSlide,
  op: PptxConnectorOp,
): void {
