/**
 * presentation PPTX export adapter tests.
 *
 * Tests the pure conversion from `ExportDeckSpec` (from `buildExportSpec`) to
 * a `PptxDeckSpec` with inch-based coordinates and diagnostics for
...
import type { ExportDeckSpec } from "@/lib/presentation/export-spec";
...
describe("buildPptxSpec — canvas format mapping", () => {
  test("16:9 maps to LAYOUT_WIDE with 13.333 × 7.5 in", () => {
    const spec = makeExportSpec();
    const pptx = buildPptxSpec(spec);
  ...
  test("4:3 maps to LAYOUT_4X3 with 10 × 7.5 in", () => {
    ...
    resetBuilderCounter();
    const deck = buildDeck([buildCoverSlide()], { canvas: canvas4x3 });
  ...
  test("square maps to LAYOUT_CUSTOM with 7.5 × 7.5 in", () => {
    ...
    resetBuilderCounter();
    const deck = buildDeck([buildCoverSlide()], { canvas: squareCanvas });
    const pkg = buildMinimalThemePackage();
    const renderTree = resolveDeckRenderTree(deck, pkg);
    const exportSpec = buildExportSpec(renderTree);
    const pptx = buildPptxSpec(exportSpec);
...
describe("buildPptxSpec — slide count and structure", () => {
  test("produces one PptxSlideSpec per ExportSlideSpec", () => {
...
describe("buildPptxSpec — coordinate conversion", () => {
...
describe("buildPptxSpec — table conversion", () => {
...
describe("buildPptxSpec — E05 image and visual fidelity", () => {
...
describe("buildPptxSpec — diagnostics", () => {
...
describe("buildPptxSpec — speaker notes", () => {
...
describe("buildPptxSpec — radial gradient and image fill fallback", () => {
...
describe("buildPptxSpec — custom canvas format", () => {
  test("custom canvas produces LAYOUT_CUSTOM", () => {
    const customCanvas: import("@/lib/presentation/types").CanvasSpec = {
      format: "custom",
      width: 120,
      height: 90,
      unit: "percent",
    ...
    resetBuilderCounter();
    const deck = buildDeck([buildCoverSlide()], { canvas: customCanvas });
    const pkg = buildMinimalThemePackage();
    const renderTree = resolveDeckRenderTree(deck, pkg);
    const exportSpec = buildExportSpec(renderTree);
    const pptx = buildPptxSpec(exportSpec);
...
describe("buildPptxSpec — visual operation coverage", () => {
  test("visual slide op is produced and carries visual content", () => {
    resetBuilderCounter();
    const deck = buildDeck([buildSlide("visual-focus", [buildVisualNode()])]);
    const pkg = buildMinimalThemePackage();
    const renderTree = resolveDeckRenderTree(deck, pkg);
    const exportSpec = buildExportSpec(renderTree);
    const pptx = buildPptxSpec(exportSpec);
...
describe("buildPptxSpec — connector operation fidelity", () => {
  test("connector op preserves endpoint, routing, dash, and arrow style", () => {
    resetBuilderCounter();
    const deck = buildDeck([buildSlide("process", [buildConnectorNode()])]);
    const pkg = buildMinimalThemePackage();
    const renderTree = resolveDeckRenderTree(deck, pkg);
    const exportSpec = buildExportSpec(renderTree);
    const pptx = buildPptxSpec(exportSpec);
...
describe("buildPptxSpec — visual fallback diagnostics", () => {
...
describe("buildPptxSpec — shared PPTX fidelity parity fixtures", () => {
...
describe("buildPptxSpec — direct operation conversion", () => {
...
describe("buildPptxSpec — direct operation conversion edge cases", () => {
