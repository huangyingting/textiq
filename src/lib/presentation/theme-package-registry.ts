...
export function getThemePackage(
  ...
  return PACKAGE_BY_ID.get(resolveThemePackageId(packageId));
}

export function listThemePackages(): readonly ThemePackageV1[] {
...
export function resolveThemePackageForDeck(
  ...
  const requestedPackageId = resolveThemePackageId(deck.theme.packageId);
  ...
  return {
    ...
    diagnostics: [
      makeDiagnostic(
        ...
        "warning",
        `Unknown presentation theme package "${requestedPackageId}". Rendering with Neutral fallback.`,
        {
