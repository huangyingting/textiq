...
import { buildDeckOutline } from "@/lib/presentation/a11y/deck-outline";
import { resolveDeckRenderTree } from "@/lib/presentation/render-resolver";
import type { ConnectorNode, GroupNode } from "@/lib/presentation/schema";
import {
  buildDeck,
  buildImageAsset,
  buildImageNode,
...
describe("buildDeckOutline", () => {
  test("builds an ordered multi-slide outline and filters decorative nodes", () => {
    ...
    const emptySlide = buildSlide("content", [], { id: "empty-slide" });
    const deck = buildDeck([firstSlide, secondSlide, emptySlide], {
      assets: {
        images: {
