...
test("InlineTextEditorPresentation mounts rich HTML, places client caret, and commits auto-height text", () => {
  withHappyDom((window) => {
    ...
    const editor = collectElements(tree).find(
      (element) =>
        (element.props as { "data-inline-editor-presentation"?: string })[
...
test("InlineTextEditorPresentation applies toolbar formatting, list, link, and unlink commands", () => {
  withHappyDom((window) => {
    const container = window.document.createElement("div");
    window.document.body.append(container);
    ...
    const dispatch = (command: string, value?: string) => {
      window.document.dispatchEvent(
        new window.CustomEvent(INLINE_TEXT_COMMAND_EVENT_, {
...
test("InlineTextEditorPresentation maps alignment commands and cancels safely without a mounted editor", () => {
