...
import type { ConnectorNode, Deck } from "@/lib/presentation/schema";
import {
  buildDeck,
  buildImageAsset,
  buildImageNode,
...
function coverageDeck() {
  ...
  return buildDeck(
    [
      slide,
      buildSlide("executive-summary", [buildTextNode({ id: "text-b" })], {
        id: "slide-b",
      }),
    ],
    {
...
function createHarness(
  overrides: Partial<SlideEditorProps> = {},
  ...
  return {
    ...
    render() {
      return renderer.run(() => SlideEditor(props()));
    },
  };
}
...
function rootProps(tree: ReactNode): ElementProps {
  return findProps(tree, (props) => props["data-slide-editor"] === "true");
}
...
test("SlideEditor direct state coverage drives toolbar, source, diagnostics, and close flows", async () => {
...
test("SlideEditor direct state coverage drives keyboard, inline edit, upload, and gestures", async () => {
  const browser = installBrowserGlobals();
  try {
    const harness = createHarness();
    let tree = harness.render();
    ...
    const inlineEditor = findProps(
      tree,
      (_props, element) =>
        typeName(element.type) === "InlineTextEditorPresentation",
    );
...
test("SlideEditor direct state coverage drives desktop toolbar branches", async () => {
...
test("SlideEditor exported dialog and inspector helpers cover direct branches", () => {
  const calls: string[] = [];
