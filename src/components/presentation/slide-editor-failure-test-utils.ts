...
export function buildEditorDeck(): Deck {
...
export function nodePointerDownFrom(
  root: ReactNode,
): NonNullable<React.ComponentProps<typeof SlideCanvas>["onNodePointerDown"]> {
...
export function focusNode(root: ReactNode, nodeId: string) {
  const onNodeFocus = (
    stageCanvasFrom(root).props as {
      onNodeFocus?: React.ComponentProps<typeof SlideCanvas>["onNodeFocus"];
    }
  ).onNodeFocus;
