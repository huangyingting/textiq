...
const canvasElement = elementFactory.createElement(
  { slideCanvas: "true", nodeId: "shape-a" },
...
test("SlideEditor final coverage drives save-first errors and unsaved close dialog", async () => {
...
test("SlideEditor final coverage drives mobile menus, notes, zoom, and picker branches", () => {
  const browser = installBrowserGlobals({ syncTimers: true });
  try {
    const harness = createHarness({}, { runEffects: true });
    let tree = harness.render();
...
test("SlideEditor final coverage drives selected source, diagnostics, file refs, and focus escape", async () => {
  const browser = installBrowserGlobals({ syncTimers: true });
  try {
    const harness = createHarness({}, { runEffects: true });
    let tree = harness.render();
...
test("SlideEditor final coverage drives clipboard grouping and context menu callbacks", () => {
  const browser = installBrowserGlobals({ syncTimers: true });
  try {
    const harness = createHarness({}, { runEffects: true });
    let tree = harness.render();
