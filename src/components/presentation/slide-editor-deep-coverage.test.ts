...
function coverageDeck(): Deck {
  return buildDeck(
    [
      ...
      buildSlide("content", [buildTextNode({ id: "body-b" })], {
        id: "slide-b",
        name: "Slide B",
      }),
    ],
...
describe("SlideEditor exported branch helpers", () => {
...
describe("SlideEditor render and interaction branches", () => {
  test("renders empty, saving, and error status shells", () => {
    const emptyHtml = renderToStaticMarkup(
      React.createElement(SlideEditor, {
        ...editorProps({ deck: buildDeck([]), onClose: undefined }),
      }),
    );
  ...
  test("closes immediately without unsaved work and opens status, source, and diagnostic menus safely", async () => {
    ...
    try {
      ...
      let tree = renderer.run(() => SlideEditor(props));
      tree = renderer.run(() => SlideEditor(props));
      ...
      const rerendered = renderer.run(() => SlideEditor(editorProps()));
  ...
  test("handles stage keyboard help and diagnostics review navigation", () => {
    ...
    try {
      ...
      let tree = renderer.run(() => SlideEditor(editorProps()));
      const root = findProps(
        tree,
        (props) => props["data-slide-editor"] === "true",
      );
      ...
      tree = renderer.run(() => SlideEditor(editorProps()));
